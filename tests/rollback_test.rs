...
async fn rolls_back_to_previous_and_removes_new_files() {
    ...
    let cfg = Config {
        deploy: wxupd::config::DeployCfg { auto: false },
        ..Config::default()
    };
    ...
    assert_eq!(
        outcome.rolled_back,
        vec![("scheme".into(), "v2".into(), "v1".into())]
    );
    assert!(
        !rime.join("v2-only.yaml").exists(),
        "v2-only.yaml should be deleted"
    );
    assert_eq!(
        std::fs::read(rime.join("shared.yaml")).unwrap(),
        b"V1 contents"
    );
...
async fn rerollback_restores_original_version() {
    ...
    assert_eq!(
        std::fs::read(rime.join("shared.yaml")).unwrap(),
        b"V2 contents",
        "shared.yaml should be back to v2"
    );
    assert!(
        rime.join("v2-only.yaml").exists(),
        "v2-only.yaml should be restored"
    );
    assert_eq!(
        std::fs::read(rime.join("v2-only.yaml")).unwrap(),
        b"V2 added"
    );
